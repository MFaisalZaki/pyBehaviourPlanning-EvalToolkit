...
def solve(args):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(dir=expdetails['tmp-dir']) as tmpdirname:
            ...
            task = replace_hyphens_in_task(task)
            # write the domain and file and update the expdetails
            domainfile = os.path.join(tmpdirname, f'renamed-{task.name}-domain.pddl')
            problemfile = os.path.join(tmpdirname, f'renamed-{task.name}-problem.pddl')
            writer = PDDLWriter(task)
            writer.write_domain(domainfile)
            writer.write_problem(problemfile)
            updatekeyvalue(expdetails, 'domainfile', domainfile)
            updatekeyvalue(expdetails, 'problemfile', problemfile)
            ...
            # return the original domain and problem files.
            updatekeyvalue(results, 'domainfile', domain)
            updatekeyvalue(results, 'problemfile', problem)
            updatekeyvalue(results, 'domain', f'{os.path.basename(os.path.dirname(domain))}/{os.path.basename(domain)}')
            updatekeyvalue(results, 'problem', os.path.basename(problem))
...
def score(args):
    ...
    try:
        with tempfile.TemporaryDirectory(dir=tmp_dump_dir) as tmpdirname:
            ...
            task = replace_hyphens_in_task(task)
            # write the domain and file and update the expdetails
            domain = os.path.join(tmpdirname, f'renamed-{task.name}-domain.pddl')
            problem = os.path.join(tmpdirname, f'renamed-{task.name}-problem.pddl')
            writer = PDDLWriter(task)
            writer.write_domain(domain)
            writer.write_problem(problem)
            
            updatekeyvalue(expdetails, 'domainfile', domain)
            updatekeyvalue(expdetails, 'problemfile', problem)
